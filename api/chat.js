export default async function handler(req, res) {
  try {
    ...
    if (!req.body || !req.body.feature || !req.body.prompt) {
    ...
    const features = {
      'Email Generator': 'Generate an email based on the following prompt: ',
      'Summarizer': 'Summarize the following text: ',
      'Rewriter': 'Rewrite the following text: ',
      'Tweet Composer': 'Compose a tweet based on the following prompt: '
    };

    const systemPrompt = `You are an AI writer. Your task is to ${features[req.body.feature]}`;
